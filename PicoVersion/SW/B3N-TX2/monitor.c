...
/* 
 * Turn ADF4351 frequency generation of/off
 */
void mon_adf4351(void)
{
	int on_off=-1;

	//printf("VFO %d %d %d %d\n",vfo._ce_initialised, vfo.pins.gpio_ce,vfo._cfreq, vfo.ClkDiv);
...
/* 
 * Change ADF4351 frequency
 */
void mon_adf4351_freq(void)
{
	uint32_t frequency=2400000000;
	char *ptr;
	if (nargs>1) 
		frequency = strtoul(argv[1],&ptr,10);
	if (frequency<ADF_FREQ_MIN||frequency>ADF_FREQ_MAX)
		return;
	printf("Target Frequency %lu\n",frequency);
	ADF4351_set_freq(&vfo,frequency);
}
 ...
 * Change ADF4351 frequency
 */
void mon_adf4351_ref(void)
{
	uint32_t ref_frequency=REF_FREQ_DEFAULT;
	char *ptr;
	if (nargs>1) 
		ref_frequency = strtoul(argv[1],&ptr,10);
	if (ref_frequency<ADF_REFIN_MIN||ref_frequency>ADF_REFIN_MAX)
		return;
	printf("Target Reference Frequency %lu\n",ref_frequency);
	ADF4351_set_ref_freq(&vfo, ref_frequency);
}
...
shell_t shell[NCMD]=
{
	...
	{"ad",  2, &mon_si,  "ad <start> <nr of reg>", "Dumps AF4351 registers"},
	...
	{"mod", 3, &mon_mode, "usb u:usb l:lsb", "Changes TX modulation type to usb or lsb"},
	{"a43", 3, &mon_adf4351, "a43 1/0 (on/off)","Turn on or off ADF4351 Frequency generation"},
	{"af",  2, &mon_adf4351_freq, "af frequency (Hz) 10 digits (0400000000 for 400 MHz)","Change ADF4351 Frequency"},
	{"ref", 3, &mon_adf4351_ref, "ref frequency (Hz) 9 digits (25000000 for 25 MHz)","Change ADF4351 referenceFrequency"}
};
